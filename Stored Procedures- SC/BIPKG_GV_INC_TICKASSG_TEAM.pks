CREATE OR REPLACE PACKAGE SCREPORT.BIPKG_Gv_INC_TICKASSG_TEAM AS
/******************************************************************************
...
END BIPKG_Gv_INC_TICKASSG_TEAM;
/
CREATE OR REPLACE PACKAGE BODY SCREPORT.BIPKG_Gv_INC_TICKASSG_TEAM AS
/******************************************************************************
   NAME:       BIPKG_INC_TICKASSG_TEAM
   PURPOSE:
...
PROCEDURE BIPKG_INC_TICKASSG_TEAM(
   ...
   BEGIN          
       ...
       v_db_zone := 'GMT';      
        ...
        v_select_stmt := 'SELECT BIPKG_UTILS.BIFNC_AdjustForTZ(v_psm.OPEN_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') OPEN_TIME, v_psm.PROBLEM_STATUS,v_psm.ASSIGNEE_NAME, v_psm.NUMBERPRGN, v_psm.BRIEF_DESCRIPTION, v_psm.PFZ_FULL_NAME, ACTIVITYM1.TYPE, ACTIVITYM1.OPERATOR, BIPKG_UTILS.BIFNC_AdjustForTZ(ACTIVITYM1.SYSMODTIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') SYSMODTIME, v_psm.PRODUCT_TYPE,v_pb.ASSIGNMENT, BIPKG_UTILS.BIFNC_AdjustForTZ( v_pb.UPDATE_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') UPDATE_TIME, v_pb.UPDATED_BY, ACTIVITYM1.NUMBERPRGN, ACTIVITYM1.THENUMBER, ACTIVITYM1.DESCRIPTION, ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay,v_psm.category';
        ...
        v_select_stmt := v_select_stmt || ' INNER JOIN ACTIVITYM1 ACTIVITYM1 ON v_pb.NUMBERPRGN = ACTIVITYM1.NUMBERPRGN AND v_pb.SYSMODTIME = ACTIVITYM1.SYSMODTIME ';

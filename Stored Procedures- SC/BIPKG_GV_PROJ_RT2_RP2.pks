CREATE OR REPLACE PACKAGE SCREPORT.BIPKG_Gv_PROJ_RT2_RP2 AS
/******************************************************************************
...
END BIPKG_Gv_PROJ_RT2_RP2;
/
CREATE OR REPLACE PACKAGE BODY SCREPORT.BIPKG_Gv_PROJ_RT2_RP2 AS
/******************************************************************************
   NAME:       BIPKG_PROJ_RT2_RP2
   PURPOSE:
...
PROCEDURE BIPKG_PROJ_RT2_RP2(
  ...
  v_db_zone := 'GMT';      
        ...
        v_select_stmt := 'SELECT v_psm.NUMBERPRGN, v_psm.LOCATION, v_psm.ASSIGNMENT, BIPKG_UTILS.BIFNC_AdjustForTZ(v_psm.CLOSE_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') CLOSE_TIME, BIPKG_UTILS.BIFNC_AdjustForTZ(v_psm.OPEN_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') OPEN_TIME, v_psm.PFZ_RELATED_PROJECTS, v_psm.BRIEF_DESCRIPTION, ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay,';

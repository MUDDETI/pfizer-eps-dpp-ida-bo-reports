CREATE OR REPLACE PROCEDURE Assets_indvl_active_decomm (
      rc_Assets_indvl_active_decomm		 IN OUT	  sys_refcursor,
...
/******************************************************************************
   ...
   ---------  ----------  ---------------  ------------------------------------
   1.0        8/21/2006   Shrikanth Rao    1. Created this procedure.
...
******************************************************************************/
   ...
   begin
      ...
      ' cm1.dept department_id_ci_owner, cm1.pfz_charge_code cost_center_ci_owner, cm1.pfz_division division_ci_owner,dm1.description brief_description, '||
	  ...
	  if pWhichCursor = 'ACTIVE' or pWhichCursor = 'DECOMM'
	  then
	  ...
	  if pWhichCursor = 'OWNER' or pWhichCursor = 'ACTIVE'
	  then
	     v_select_stmt := v_select_stmt ||
         ' and dm1.istatus in ('||''''||'Run'||''''||','||''''||'run'||''''||','||''''||'New'||''''||','||
         ''''||'RUN'||''''||','||''''||'Run - Update'||''''||')';
	  elsif pWhichCursor = 'DECOMM'
	  then
	     v_select_stmt := v_select_stmt ||
		 ' and dm1.istatus = '||''''||'Decommissioned'||'''';
	  end if;
      open rc_Assets_indvl_active_decomm for v_select_stmt;

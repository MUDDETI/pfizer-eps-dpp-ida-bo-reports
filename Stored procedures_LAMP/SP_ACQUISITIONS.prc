CREATE OR REPLACE PROCEDURE SP_ACQUISITIONS (
      rc_Acquisitions		 			 IN OUT	  sys_refcursor,
...
******************************************************************************/
   ...
   begin
	  ...
	  ' decode(nvl(far.lifeyrs,0),0,0,nvl(substr(far.cost,2,length(far.cost)),0)/nvl(far.lifeyrs,0)) Impact_depr_expense_rr';
	  v_select_stmt := v_select_stmt ||
      ...
      ''''||'RUN'||''''||','||''''||'Run - Update'||''''||')'||
	  ' and dm1.pfz_purchase_date between '||''''||v_gmt_startdate||''''||' and '||''''||v_gmt_enddate||'''';
      open rc_Acquisitions for v_select_stmt;

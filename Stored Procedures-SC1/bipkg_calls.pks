CREATE OR REPLACE PACKAGE Bipkg_Calls
AS
...
END Bipkg_Calls;
/
CREATE OR REPLACE PACKAGE BODY Bipkg_Calls
AS
/******************************************************************************
...
--
   ...
   BEGIN
		...
		'AFFECTED_ITEM, OWNER_NAME, OPEN, CALLBACK_TYPE, CALLBACK_REASON, NULLPRGN, ' ||
   		...
   		'MANDATORY_ASSET, FLOOR, BUILDING, VARIABLE1, VARIABLE2, VARIABLE3, CORP_STRUCTURE, ' ||
   		'CORP_LEVEL1, CORP_LEVEL2, CORP_LEVEL3, CONTACT_EMAIL, LOCATION_FULL_NAME, ' ||
...
--		  	Created bisp_select_calls_spflg 
   ...
   BEGIN
		...
		'FIRST_CALL, BRIEF_DESCRIPTION, PFZ_DIVISION, PFZ_BU, PFZ_SPECIAL_PROJECT,  ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay';
  ...
  OPEN select_spflg_cursor FOR v_select_stmt ;
   ...
   PROCEDURE bisp_select_FCR(
	  ...
	  poverride			 IN		  VARCHAR2,
	  pzone				 IN		  VARCHAR2,
	  pstartdate         IN       DATE,                 -- For use in case of adhoc reports.
      penddate           IN       DATE,
	  ptype				 IN		  VARCHAR2                 -- For use in case of adhoc reports.
   ) IS
   -- Grouping Types: 
  ...
  -- RMakkena 19-OCT-2006 created procedure for Web Call Analysis Report.   
   ...
   BEGIN          
	    ...
	    v_startdatedisplay   := TO_CHAR (Bipkg_Utils.bifnc_adjustfortz (v_gmt_startdate,v_db_zone,pzone),'DD-MM-YYYY HH24:MI:SS');
        v_enddatedisplay     := TO_CHAR (Bipkg_Utils.bifnc_adjustfortz (v_gmt_enddate,v_db_zone,pzone),'DD-MM-YYYY HH24:MI:SS');
		v_select_stmt := 'SELECT INCIDENTSM1.INCIDENT_ID, BIPKG_UTILS.BIFNC_AdjustForTZ( INCIDENTSM1.OPEN_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') OPEN_TIME, INCIDENTSM1.PFZ_CALL_SOURCE, INCIDENTSM1.OPENED_BY, PROBSUMMARYM1.NUMBERPRGN, PROBSUMMARYM1.PFZ_SITE_ID, INCIDENTSM1.LOCATION, INCIDENTSM1.PFZ_ORIG_GROUP, ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay';
        v_select_stmt := v_select_stmt || ' FROM INCIDENTSM1 INCIDENTSM1 ' ;

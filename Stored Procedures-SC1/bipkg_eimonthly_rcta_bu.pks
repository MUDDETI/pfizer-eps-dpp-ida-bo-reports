CREATE OR REPLACE PACKAGE BIPKG_EIMONTHLY_RCTA_BU AS
/******************************************************************************
...
/
CREATE OR REPLACE PACKAGE BODY BIPKG_EIMONTHLY_RCTA_BU AS
/******************************************************************************
   NAME:       BIPKG_EIMONTHLY_RCTA_BU
   PURPOSE:
...
PROCEDURE BIPKG_EIMONTHLY_RCTA_BU(
   ...
   BEGIN          
		...
		v_select_stmt := ' SELECT PROBSUMMARYM1.ASSIGNMENT, PROBSUMMARYM1.OPEN_GROUP, BIPKG_UTILS.BIFNC_AdjustForTZ(PROBSUMMARYM1.CLOSE_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') CLOSE_TIME, PROBSUMMARYM1.NUMBERPRGN, BIPKG_UTILS.BIFNC_AdjustForTZ( PROBSUMMARYM1.OPEN_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') OPEN_TIME, PROBSUMMARYM1.PFZ_SLA_TITLE, PROBSUMMARYM1.PROBLEM_TYPE, PROBSUMMARYM1.CLOSED_BY, PROBSUMMARYM1.PRODUCT_TYPE, PROBSUMMARYM1.PFZ_BU, PROBSUMMARYM1.PFZ_DIVISION, PROBSUMMARYM1.PFZ_CHARGE_CODE, ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay';

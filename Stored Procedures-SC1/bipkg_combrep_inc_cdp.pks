CREATE OR REPLACE PACKAGE BIPKG_COMBREP_INC_CDP AS
/******************************************************************************
...
/
CREATE OR REPLACE PACKAGE BODY BIPKG_COMBREP_INC_CDP AS
/******************************************************************************
   NAME:       BIPKG_COMBREP_INC_CDP
   PURPOSE:
...
PROCEDURE BIPKG_COMBREP_INC_CDP(
   ...
   BEGIN          
        ...
        v_select_stmt := ' SELECT PROBSUMMARYM1.ASSIGNMENT, BIPKG_UTILS.BIFNC_AdjustForTZ( PROBSUMMARYM1.CLOSE_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') CLOSE_TIME,PROBSUMMARYM1.OPEN_GROUP,PROBSUMMARYM1.NUMBERPRGN,BIPKG_UTILS.BIFNC_AdjustForTZ( PROBSUMMARYM1.OPEN_TIME,'|| '''' || v_db_zone || '''' || ',' || '''' || pzone || '''' || ') OPEN_TIME,PROBSUMMARYM1.PFZ_TOTAL_TIME_SPENT,PROBSUMMARYM1.FLAG,PROBSUMMARYM1.PRODUCT_TYPE,PROBSUMMARYM1.COUNTRY, PROBSUMMARYM1.PFZ_BU, ' || '''' || v_startdatedisplay || '''' || 'StartDateDisplay , ' || '''' || v_enddatedisplay || '''' || ' EndDateDisplay';

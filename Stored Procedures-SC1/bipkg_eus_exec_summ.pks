CREATE OR REPLACE PACKAGE Bipkg_Eus_Exec_Summ
AS
...
END Bipkg_Eus_Exec_Summ;
/
CREATE OR REPLACE PACKAGE BODY Bipkg_Eus_Exec_Summ
AS
   ...
   BEGIN
	 ...
	 --TimeZone of the Database
	  ...
	  v_select_stmt := '   select'
					...
					|| ' , probsummarym1.cause_code'
					|| ' , probsummarym1.problem_type'
					|| ' , probsummarym1.product_type'
					|| ' , probsummarym1.pfz_special_project'
					|| ' , probsummarym1.pfz_division'
					|| ' , probsummarym1.pfz_bu'
					|| ' , probsummarym1.pfz_sla_title'
					|| ' , probsummarym1.pfz_resolve_sla'
					|| ' , probsummarym1.user_priority'
					|| ' , probsummarym1.updated_by'
					|| ' , probsummarym1.pfz_total_time_spent'
					|| ' , bipkg_utils.bifnc_adjustfortz (probsummarym1.open_time,''' || v_db_zone || ''', ''' || p_zone || ''') open_time'
					|| ' , bipkg_utils.bifnc_adjustfortz (probsummarym1.close_time,''' || v_db_zone || ''', ''' || p_zone || ''') close_time'
					|| ' , bipkg_eus_exec_summ.bifnc_srv_evnt_str (probsummarym1.resolution_code, probsummarym1.cause_code, probsummarym1.problem_type, probsummarym1.product_type, probsummarym1.pfz_special_project) service_event_string' 
					|| ' , ' || v_startdatedisplay || ' startdatedisplay'
					|| ' , ' || v_enddatedisplay || ' enddatedisplay'
					|| ' from'
					|| ' probsummarym1'
 					|| ' where'
					|| ' probsummarym1.pfz_sla_title <> ''Project'''					
 					|| ' and (' || Bipkg_Utils.bifnc_createnoncsvinlist ('NVL(probsummarym1.assignment, '' '')', p_service_groups, ';') || ')'
					|| ' and (' || Bipkg_Utils.bifnc_createnoncsvinlist ('NVL(probsummarym1.assignment, '' '')', p_assignment_group, ';') || ')'
					|| ' and (' || Bipkg_Utils.bifnc_createnoncsvinlist ('NVL(probsummarym1.updated_by, '' '')', p_analyst, ';') || ')'
					|| ' and (' || Bipkg_Utils.bifnc_createnoncsvinlist ('NVL(probsummarym1.pfz_division, '' '')', p_division, ';') || ')'							  			  			
	  				|| ' and'
					|| ' ('
					|| ' ('
					|| ' probsummarym1.close_time between ' || v_gmt_startdate_str || ' and ' || v_gmt_enddate_str
					|| ' and bipkg_eus_exec_summ.bifnc_srv_evnt_str (probsummarym1.resolution_code, probsummarym1.cause_code, probsummarym1.problem_type, probsummarym1.product_type, probsummarym1.pfz_special_project) like ''' || v_srv_str || '''' 
					|| ' )'
					;
      ...
      ELSE									
									v_select_stmt := v_select_stmt
	  				   				|| ' or'
									|| ' ('
									|| ' probsummarym1.open_time <= ' || v_gmt_enddate_str 
									|| ' and'
									|| ' ('
									|| ' probsummarym1.flag = ''t'''
									|| ' or'
									|| ' probsummarym1.close_time > ' || v_gmt_enddate_str
									|| ' )'
									|| ' )'
									|| ' )'
									;
